    ...
    <main className="min-h-screen bg-[#f8f7f3] text-[#17231d]">
      ...
      <section className="mx-auto max-w-7xl px-6 py-16 lg:px-8 lg:py-20">
        <div className="max-w-3xl">
          <p className="text-sm font-bold uppercase tracking-[0.18em] text-[#7a9b76]">Pronostics</p>
          <h1 className="mt-3 text-4xl font-black tracking-[-0.04em] sm:text-6xl">Les pronostics</h1>
          <p className="mt-5 text-lg leading-8 text-[#5d6962]">Les pronostics sont automatiquement alimentés depuis Supabase et classés par date.</p>
        </div>
        ...
        {error ? (
          ...
          <div className="mt-12 space-y-14">
            {days.map(([dateKey, dayPredictions]) => (
              <section key={dateKey}>
                <div className="mb-6 flex items-end justify-between gap-4 border-b border-[#17231d]/10 pb-4">
                  <div><p className="text-sm font-bold uppercase tracking-[0.18em] text-[#7a9b76]">Date</p><h2 className="mt-1 text-2xl font-black capitalize sm:text-3xl">{formatDate(dayPredictions[0].start_date!)}</h2></div>
                  <span className="rounded-full bg-[#17231d] px-4 py-2 text-xs font-bold text-white">{dayPredictions.length} pronostic{dayPredictions.length > 1 ? "s" : ""}</span>
                </div>
                <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
                  {dayPredictions.map((match) => {
                    const odd = getPredictionOdd(match.prediction, match);
                    return (
                      <article key={match.id} className="rounded-3xl border border-[#17231d]/10 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl hover:shadow-[#17231d]/5">
                        <div className="flex items-center justify-between text-xs font-bold uppercase tracking-wider"><span className="text-[#7a9b76]">{match.competition_name ?? match.competition_cluster ?? "Football"}</span><span className="rounded-full bg-[#eef3eb] px-3 py-1 text-[#45604b]">À venir</span></div>
                        <p className="mt-5 text-sm font-semibold text-[#68736d]">Coup d&apos;envoi : {formatTime(match.start_date!)}</p>
                        <div className="mt-5 space-y-3"><div className="rounded-2xl bg-[#f5f5f1] px-5 py-4 text-center text-lg font-black">{match.home_team}</div><div className="text-center text-xs font-black text-[#7a9b76]">VS</div><div className="rounded-2xl bg-[#f5f5f1] px-5 py-4 text-center text-lg font-black">{match.away_team}</div></div>
                        <div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-[#eef3eb] p-4 text-center"><p className="text-xs font-bold uppercase tracking-wider text-[#68736d]">Pronostic</p><p className="mt-1 text-2xl font-black">{match.prediction ?? "-"}</p></div><div className="rounded-2xl bg-[#eef3eb] p-4 text-center"><p className="text-xs font-bold uppercase tracking-wider text-[#68736d]">Cote</p><p className="mt-1 text-2xl font-black">{odd ?? "-"}</p></div></div>
                        <Link href={`/matchs/${match.id}`} className="mt-6 block rounded-xl bg-[#17231d] px-4 py-3 text-center text-sm font-bold text-white transition hover:bg-[#26372c]">Voir le pronostic →</Link>
                      </article>
                    );
                  })}
                </div>
